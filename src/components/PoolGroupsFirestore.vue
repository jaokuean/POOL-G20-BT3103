<template>
    <div id='mainComponent'>
        <div id='searchBox'>
            <img v-bind:src="search"/> <input type="text" id="poolName" v-model="poolName" v-on:keyup.enter="fetchItems">
        </div>
...
export default {
    data: function() {
        return {
            ...
            imgURL: 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTF64AHSDlHzKv40EOHnfqiGy2ZtS3ec18Vrw&usqp=CAU',
            groups: [],
            serviceID: ""
        }
    },
    methods: {
        fetchItems: function() {
            ...
            database.firestore().collection("pools").where("serviceId","==",this.serviceID).get().then((querySnapShot)=>{
                if (querySnapShot.empty) {
                    this.hasNoGroup = true
                }
                querySnapShot.forEach((doc) => {
                    let name = doc.data().poolName
                    let desc = "There are " + doc.data().remaining + " position(s) left"
        ...
        fetchData: function() {
            ...
            database.firestore().collection("services").doc(this.$route.params.document_id).get().then(
                (doc) => {
                    //get the poolName
            ...
            //alert(this.imgURL)
            //let tempGroups = [];
            database.firestore().collection("pools").where("serviceId","==",this.$route.params.document_id).get().then((querySnapShot)=>{
                if (querySnapShot.empty) {
                    this.hasNoGroup = true
                }
            ...
            }).then(()=>{var parsedobj = JSON.parse(JSON.stringify(this.groups));console.log(parsedobj);
})
            /*var arrayLength = this.groups.length;
            for (var i = 0; i < arrayLength; i++) {
                alert(this.groups[i].groupName);
        ...
        join: function(document_id) {
            ...
            let items = database.firestore().collection("pools").doc(document_id).get().data()
            items.remaining = items.remaining - 1
            database.firestore().collection("pools").doc(document_id).update(items)
            alert("Congratulations! You've successfully joined this group! Your pool name is " + items.poolName)
        },
        ...
        create: function() {
            ...
            items['sharedUserName'] = Math.random().toString(16).substr(2, 8) + "@gmail.com"
            console.log(items)
            database.firestore().collection("pools").add(items) //.then(() => {location.reload()})
            alert("You've successfully created a group! The pool's name is: " + items.poolName + ". Wait for others to join the pool!")
        }
    },
    beforeCreate() {
        console.log("a")
